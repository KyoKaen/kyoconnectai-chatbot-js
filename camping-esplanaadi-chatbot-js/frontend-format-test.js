class CampingChatbot {
  ...
  addMessage(text, sender) {
    ...
    // If it's from the bot, parse to a user-friendly HTML format
    const displayedText = (sender === 'bot') ? this.parseMessage(text) : text;
  ...
  parseMessage(rawText) {
    let html = rawText;

    // 1. Convert **bold** to <strong>
    html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');

    // 2. Convert [Link Text](URL) to <a href="URL" target="_blank">
    html = html.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, '<a href="$2" target="_blank">$1</a>');

    // 3. Convert lines starting with - or • into <li>
    const hasBullets = /(^|\n)([-•]\s+)/.test(html);
    if (hasBullets) {
      html = html.replace(/(^|\n)([-•])\s+(.*)/g, '$1<li>$3</li>');
      // Wrap <li> items in a <ul> (naive approach)
      html = `<ul>${html}</ul>`;
      // Remove leftover newlines inside the <ul>
      html = html.replace(/\n/g, '');
    }

    // 4. Highlight phone numbers with <a href="tel:...">
    //    e.g. +372 58229262 -> <a href="tel:+372 58229262">+372 58229262</a>
    html = html.replace(
      /(\+\d{1,4}\s?\d[\d\s-]{3,15}\d)/g,
      '<a href="tel:$1">$1</a>'
    );

    // 5. Convert emails to mailto links
    html = html.replace(
...
if (document.readyState === 'complete') {
  ...
  window.addEventListener('DOMContentLoaded', () => new CampingChatbot({
    apiUrl: 'https://camping-chatbot-1096582767898.europe-west1.run.app/chat'
  }));
}
