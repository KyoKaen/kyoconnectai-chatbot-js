class CampingChatbot {
  ...
  static config = {
    ...
    FREQUENT_QUESTIONS: [
      ...
      // "Price", "Contact",
      // "What is the location of Camping Lyon?", 
      // "What are the accommodation options at Camping Lyon?", 
      // "What activities and amenities are available at Camping Lyon?", 
      "What is the price for staying at Camping Lyon?",
      "How can I contact Camping Lyon for bookings or inquiries?"
    ],
    COPY: {
      header: "Camping De Lyon AI Assistant",
      systemMessage: "Please note: You can ask up to <strong>20 questions</strong> about Camping de Lyon",
      initialMessage: "Hi! What can I help you with Camping de Lyon?",
  ...
  injectStyles() {
    ...
    style.textContent = `
      ...
      #camping-de-lyon-chatbot-container {
        position: fixed;
        bottom: 90px;
        right: 25px;
        width: 420px;
      ...
      #camping-de-lyon-chatbot-header {
        display: flex;
        align-items: center;
        padding: 10px;
      ...
      #camping-de-lyon-chatbot-header h4 {
          margin: 0;
          font-size: 1.1em;
          color: var(--primary-color); //black
      }

      #camping-de-lyon-chatbot-messages {
          flex: 1;
          padding: 10px;
          overflow-y: auto;
      ...
      #camping-de-lyon-chatbot-frequent-questions {
          padding: 10px;
          background: #fff;
          border-top: 1px solid var(--secondary-color);
          border-bottom: 1px solid var(--secondary-color);
      }
      #camping-de-lyon-chatbot-frequent-questions h6 {
      ...
      #camping-de-lyon-chatbot-input-area {
          display: flex;
          padding: 10px;
          border-top: 1px solid var(--secondary-color);
          background: #fff;
      }
      #camping-de-lyon-chatbot-input {
          flex: 1;
          padding: 8px;
          border: 1px solid #ccc;
          border-radius: 4px;
      }
      #camping-de-lyon-chatbot-send {
          background: var(--primary-color);
          color: white;
          border: none;
          padding: 8px 12px;
      ...
      /* Footer */
      #camping-de-lyon-chatbot-footer {
          text-align: center;
          font-size: 0.8em;
          color: #6c757d;
          padding: 8px;
      ...
      #camping-de-lyon-chatbot-footer a {
          color: var(--primary-color);
  ...
  createDOM() {
    ...
    this.container.innerHTML = `
      ...
      <div id="camping-de-lyon-chatbot-frequent-questions">
  ...
  initEventListeners() {
    ...
    document.getElementById('camping-de-lyon-chatbot-send').addEventListener('click', () => this.handleSend());
    document.getElementById('camping-de-lyon-chatbot-input').addEventListener('keypress', e => {
  ...
  async handleSend() {
    ...
    const input = document.getElementById('camping-de-lyon-chatbot-input');
    ...
    try {
      ...
      this.container.querySelector('#camping-de-lyon-chatbot-messages').appendChild(loading);
      ...
      const response = await fetch(this.apiEndpoint, {
        ...
        body: JSON.stringify({
          message: message,
          session_id: "camping-de-lyon-chatbot"
        })
      });
  ...
  toggleUIState(disabled) {
    const elements = [
      document.getElementById('camping-de-lyon-chatbot-send'),
  ...
  addMessage(text, sender) {
    const messagesDiv = this.container.querySelector('#camping-de-lyon-chatbot-messages');
  ...
  initFrequentQuestions() {
    ...
    CampingChatbot.config.FREQUENT_QUESTIONS.forEach(question => {
      ...
      button.addEventListener('click', () => {
        if (this.state.isProcessing) return;
        document.getElementById('camping-de-lyon-chatbot-input').value = question;
        this.handleSend();
      });
