class EnjadeChatbot {
  ...
  injectStyles() {
    ...
    style.textContent = `
      :root {
        --primary-color: ${EnjadeChatbot.config.PRIMARY_COLOR};
        --secondary-color: ${EnjadeChatbot.config.SECONDARY_COLOR};
        --user-message-bg-color: ${EnjadeChatbot.config.USER_MESSAGE_BG}; // end user message color
      }
  ...
  createDOM() {
    ...
    this.container.innerHTML = `
      <div id="kyoconnectai-hp-chatbot-header">
        <img src="${EnjadeChatbot.config.BOT_ICON}" alt="Chatbot Logo" width="40" height="40" style="border-radius:50%; margin-right:10px;">
        <h4>${EnjadeChatbot.config.COPY.header}</h4>
      </div>

      <div id="kyoconnectai-hp-chatbot-messages">
        <div class="system-message">
          ${EnjadeChatbot.config.COPY.systemMessage}
        </div>
        <div class="message-container bot-message-container">
          <img src="${EnjadeChatbot.config.BOT_ICON}" class="message-icon" alt="Bot Icon">
          <div class="message-bubble bot-message-bubble">
            ${EnjadeChatbot.config.COPY.initialMessage}
          </div>
        </div>
      </div>
      ...
      <div id="kyoconnectai-hp-chatbot-input-area">
        <input type="text" id="kyoconnectai-hp-chatbot-input" placeholder="${EnjadeChatbot.config.COPY.inputPlaceholder}">
        <button id="kyoconnectai-hp-chatbot-send">Send</button>
      </div>

      <div id="kyoconnectai-hp-chatbot-footer">
        ${EnjadeChatbot.config.COPY.footerHTML}
      </div>
    `;
    ...
    this.toggleButton.innerHTML = `
      <img src="${EnjadeChatbot.config.BOT_ICON}" alt="Chatbot Logo">  //customized toggle button
  ...
  async handleSend() {
    ...
    if (message.length > EnjadeChatbot.config.behavior.maxContentLength) {
      this.addSystemMessage(EnjadeChatbot.config.behavior.limitMessages.lengthExceeded);
      input.value = '';
      return;
    }

    if (this.state.questionCount >= EnjadeChatbot.config.behavior.maxQuestions) {
      this.addSystemMessage(EnjadeChatbot.config.behavior.limitMessages.questionLimit);
      input.value = '';
      return;
    }
  ...
  addMessage(text, sender) {
    ...
    message.innerHTML = `
      <img src="${sender === 'user' ? EnjadeChatbot.config.USER_ICON : EnjadeChatbot.config.BOT_ICON}"
           class="message-icon"
           alt="${sender} icon">
  ...
  createLoading() {
    ...
    loading.innerHTML = `
      <img src="${EnjadeChatbot.config.BOT_ICON}" class="message-icon" alt="Loading">
  ...
  initFrequentQuestions() {
    ...
    EnjadeChatbot.config.FREQUENT_QUESTIONS.forEach(question => {
...
const apiUrl = "https://enjade-fr-1096582767898.europe-west1.run.app/chat";

if (document.readyState === 'complete') {
  new EnjadeChatbot({ apiUrl });
} else {
  window.addEventListener('DOMContentLoaded', () => new EnjadeChatbot({ apiUrl }));
}
