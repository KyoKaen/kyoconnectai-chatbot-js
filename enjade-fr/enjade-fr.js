const EnjadeName = "Enjade"; // You can still change this easily
...
class EnjadeChabot {
 ...
 static config = {
   ...
   // USER_ICON: 'https://kyox.ai/assets/kyoconnectai_logo.jpg', // Keep this if needed elsewhere, not used for user messages anymore
...
// <link rel="icon" type="image/x-icon" href="https://images.squarespace-cdn.com/content/v1/6782c81bc9e926610d7efa9d/eb289d11-869a-4c50-92c6-7b4de1c72eeb/favicon.ico?format=100w">
   ...
   COPY: {
     header: `${EnjadeName} AI assistant`,
     systemMessage: `Please note: You can ask up to <strong>20 questions</strong> about ${EnjadeName}`,
     initialMessage: `Hi! What can I help you with ${EnjadeName}?`,
 ...
 injectStyles() {
   ...
   style.textContent = `
     :root {
       --primary-color: ${EnjadeChabot.config.PRIMARY_COLOR};
       --secondary-color: ${EnjadeChabot.config.SECONDARY_COLOR};
       --user-message-bg-color: ${EnjadeChabot.config.USER_MESSAGE_BG}; // end user message color
     }
 ...
 createDOM() {
   ...
   this.container.innerHTML = `
     <div id="chatbot-header">
       <img src="${EnjadeChabot.config.BOT_ICON}" alt="Chatbot Logo" width="40" height="40" style="border-radius:50%; margin-right:10px;">
       <h4>${EnjadeChabot.config.COPY.header}</h4>
     </div>

     <div id="chatbot-messages">
       <div class="system-message">
         ${EnjadeChabot.config.COPY.systemMessage}
       </div>
       <div class="message-container bot-message-container">
         <img src="${EnjadeChabot.config.BOT_ICON}" class="message-icon" alt="Bot Icon">
         <div class="message-bubble bot-message-bubble">
           ${EnjadeChabot.config.COPY.initialMessage}
         </div>
       </div>
     </div>
     ...
     <div id="chatbot-input-area">
       <input type="text" id="chatbot-input" placeholder="${EnjadeChabot.config.COPY.inputPlaceholder}">
       <button id="chatbot-send">Send</button>
     </div>

     <div id="chatbot-footer">
       ${EnjadeChabot.config.COPY.footerHTML}
     </div>
   `;
 ...
 async handleSend() {
   ...
   if (message.length > EnjadeChabot.config.behavior.maxContentLength) {
     this.addSystemMessage(EnjadeChabot.config.behavior.limitMessages.lengthExceeded);
     input.value = '';
     return;
   }

   if (this.state.questionCount >= EnjadeChabot.config.behavior.maxQuestions) {
     this.addSystemMessage(EnjadeChabot.config.behavior.limitMessages.questionLimit);
     input.value = '';
     return;
   }
 ...
 createLoading() {
   ...
   loading.innerHTML = `
     <img src="${EnjadeChabot.config.BOT_ICON}" class="message-icon" alt="Loading">
 ...
 initFrequentQuestions() {
   ...
   EnjadeChabot.config.FREQUENT_QUESTIONS.forEach(question => {
...
if (document.readyState === 'complete' || document.readyState === 'interactive') {
 new EnjadeChabot({ apiUrl });
} else {
 window.addEventListener('DOMContentLoaded', () => new EnjadeChabot({ apiUrl }));
}
