class LourdesELPChatbot {
  ...
  injectStyles() {
    ...
    style.textContent = `
      :root {
        --primary-color: ${LourdesELPChatbot.config.PRIMARY_COLOR};
        --secondary-color: ${LourdesELPChatbot.config.SECONDARY_COLOR};
        --user-message-bg-color: ${LourdesELPChatbot.config.USER_MESSAGE_BG}; // end user message color
      }
  ...
  createDOM() {
    ...
    this.container.innerHTML = `
      <div id="kyoconnectai-hp-chatbot-header">
        <img src="${LourdesELPChatbot.config.BOT_ICON}" alt="Chatbot Logo" width="40" height="40" style="border-radius:50%; margin-right:10px;">
        <h4>${LourdesELPChatbot.config.COPY.header}</h4>
      </div>

      <div id="kyoconnectai-hp-chatbot-messages">
        <div class="system-message">
          ${LourdesELPChatbot.config.COPY.systemMessage}
        </div>
        <div class="message-container bot-message-container">
          <img src="${LourdesELPChatbot.config.BOT_ICON}" class="message-icon" alt="Bot Icon">
          <div class="message-bubble bot-message-bubble">
            ${LourdesELPChatbot.config.COPY.initialMessage}
          </div>
        </div>
      </div>
      ...
      <div id="kyoconnectai-hp-chatbot-input-area">
        <input type="text" id="kyoconnectai-hp-chatbot-input" placeholder="${LourdesELPChatbot.config.COPY.inputPlaceholder}">
        <button id="kyoconnectai-hp-chatbot-send">Send</button>
      </div>

      <div id="kyoconnectai-hp-chatbot-footer">
        ${LourdesELPChatbot.config.COPY.footerHTML}
      </div>
    `;
    ...
    this.toggleButton.innerHTML = `
      <img src="${LourdesELPChatbot.config.BOT_ICON}" alt="Chatbot Logo">  //customized toggle button
  ...
  async handleSend() {
    ...
    if (message.length > LourdesELPChatbot.config.behavior.maxContentLength) {
      this.addSystemMessage(LourdesELPChatbot.config.behavior.limitMessages.lengthExceeded);
      input.value = '';
      return;
    }

    if (this.state.questionCount >= LourdesELPChatbot.config.behavior.maxQuestions) {
      this.addSystemMessage(LourdesELPChatbot.config.behavior.limitMessages.questionLimit);
      input.value = '';
      return;
    }
  ...
  addMessage(text, sender) {
    ...
    message.innerHTML = `
      <img src="${sender === 'user' ? LourdesELPChatbot.config.USER_ICON : LourdesELPChatbot.config.BOT_ICON}"
           class="message-icon"
           alt="${sender} icon">
  ...
  createLoading() {
    ...
    loading.innerHTML = `
      <img src="${LourdesELPChatbot.config.BOT_ICON}" class="message-icon" alt="Loading">
  ...
  initFrequentQuestions() {
    ...
    LourdesELPChatbot.config.FREQUENT_QUESTIONS.forEach(question => {
...
if (document.readyState === 'complete') {
  new LourdesELPChatbot({ apiUrl });
} else {
  window.addEventListener('DOMContentLoaded', () => new LourdesELPChatbot({ apiUrl }));
}
