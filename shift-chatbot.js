class ShiftIncChatbot {

  // =============== Configuration ===============
  static config = {
    PRIMARY_COLOR: '#000000',
    SECONDARY_COLOR: '#F0F4F8',
    USER_MESSAGE_BG: '#b8302b', //shift red
    USER_ICON: 'https://kyoconnectai.com/kyoconnectai_logo.jpg',
    // CUSTOMIZED_ICON: 'https://kyoconnectai.com/camping-esplanaadi-logo.jpg',
    CUSTOMIZED_ICON: 'https://www.cdn.shiftinc.jp/assets/favicon.ico" type="image/vnd.microsoft.icon',
    BOT_ICON: 'https://www.cdn.shiftinc.jp/assets/favicon.ico" type="image/vnd.microsoft.icon',
    ...
    COPY: {
      ...
      systemMessage: "Please note: You can ask up to <strong>20 questions</strong> about SHIFT",
      initialMessage: "Hi! What can I help you with SHIFT?",
  ...
  injectStyles() {
    ...
    style.textContent = `
      :root {
        --primary-color: ${ShiftIncChatbot.config.PRIMARY_COLOR};
        --secondary-color: ${ShiftIncChatbot.config.SECONDARY_COLOR};
        --user-message-bg-color: ${ShiftIncChatbot.config.USER_MESSAGE_BG}; // end user message color
      }
  ...
  createDOM() {
    ...
    this.container.innerHTML = `
      <div id="kyoconnectai-hp-chatbot-header">
        <img src="${ShiftIncChatbot.config.BOT_ICON}" alt="Chatbot Logo" width="40" height="40" style="border-radius:50%; margin-right:10px;">
        <h4>${ShiftIncChatbot.config.COPY.header}</h4>
      </div>

      <div id="kyoconnectai-hp-chatbot-messages">
        <div class="system-message">
          ${ShiftIncChatbot.config.COPY.systemMessage}
        </div>
        <div class="message-container bot-message-container">
          <img src="${ShiftIncChatbot.config.BOT_ICON}" class="message-icon" alt="Bot Icon">
          <div class="message-bubble bot-message-bubble">
            ${ShiftIncChatbot.config.COPY.initialMessage}
          </div>
        </div>
      </div>
      ...
      <div id="kyoconnectai-hp-chatbot-input-area">
        <input type="text" id="kyoconnectai-hp-chatbot-input" placeholder="${ShiftIncChatbot.config.COPY.inputPlaceholder}">
        <button id="kyoconnectai-hp-chatbot-send">Send</button>
      </div>

      <div id="kyoconnectai-hp-chatbot-footer">
        ${ShiftIncChatbot.config.COPY.footerHTML}
      </div>
    `;
    ...
    this.toggleButton.innerHTML = `
      <img src="${ShiftIncChatbot.config.BOT_ICON}" alt="Chatbot Logo">  //customized toggle button
  ...
  async handleSend() {
    ...
    if (message.length > ShiftIncChatbot.config.behavior.maxContentLength) {
      this.addSystemMessage(ShiftIncChatbot.config.behavior.limitMessages.lengthExceeded);
      input.value = '';
      return;
    }

    if (this.state.questionCount >= ShiftIncChatbot.config.behavior.maxQuestions) {
      this.addSystemMessage(ShiftIncChatbot.config.behavior.limitMessages.questionLimit);
      input.value = '';
      return;
    }
  ...
  addMessage(text, sender) {
    ...
    message.innerHTML = `
      <img src="${sender === 'user' ? ShiftIncChatbot.config.USER_ICON : ShiftIncChatbot.config.BOT_ICON}"
           class="message-icon"
           alt="${sender} icon">
  ...
  createLoading() {
    ...
    loading.innerHTML = `
      <img src="${ShiftIncChatbot.config.BOT_ICON}" class="message-icon" alt="Loading">
  ...
  initFrequentQuestions() {
    ...
    ShiftIncChatbot.config.FREQUENT_QUESTIONS.forEach(question => {
...
const apiUrl = "https://shift-inc-1096582767898.europe-west1.run.app/chat";

if (document.readyState === 'complete') {
  new ShiftIncChatbot({ apiUrl });
} else {
  window.addEventListener('DOMContentLoaded', () => new ShiftIncChatbot({ apiUrl }));
}
